...
public class SceneTaskServiceImpl implements SceneTaskService {
    ...
    @Resource(type = EngineService.class)
    ...
    private SceneActionResp startTask(SceneTaskStartInput input) {
        ...
        SceneActionResp sceneAction = new SceneActionResp();
        ...
        // 清除SLA条件缓存
        redisTemplate.opsForHash().delete(SlaServiceImpl.SLA_SCENE_KEY, String.valueOf(input.getSceneId()));
        //设置缓存，用以检查压测场景启动状态
    ...
    @Override
    public SceneActionResp checkSceneTaskStatus(Long sceneId, Long reportId) {
        //为如果传入报告id，以报告id为准
        SceneActionResp scene = new SceneActionResp();
    ...
    public Long startFlowDebugTask(SceneManageWrapperInput input, List<EnginePluginInput> enginePlugins) {
        ...
        if (sceneManageResult == null) {
            ...
            SceneSlaRefResponse sceneSlaRefInput = new SceneSlaRefResponse();
