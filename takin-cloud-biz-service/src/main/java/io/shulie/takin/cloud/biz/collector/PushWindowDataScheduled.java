        ...
        results.stream().filter(Objects::nonNull)
            .map(r -> (Runnable)() -> {
                ...
                try {
                    ...
                    SceneManageWrapperOutput scene;
                    // 如果数据库查询不到场景，会报错，此时要保证报告结束
                    try {
                        scene = sceneManageService.getSceneManage(sceneId, null);
                        ...
                        log.error("push data error,force close task！ errorMsg:{}", e.getMessage());
                        forceClose(CollectorUtil.getNowTimeWindow(), sceneId, reportId, customerId);
                        return;
                    }
